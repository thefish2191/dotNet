...
static void GenerateCsvFile(IQueryable<Category> categories)
{
    ...
    using (FileStream csvStream = File.Create(Combine(CurrentDirectory, csvFile)))
    {
        ...
        {
            csv.WriteLine(
                "CategoryId,CategoryName,Description,ProductId,ProductName,Cost,Stock,Discontinued"
            );
            ...
            foreach (Category c in categories)
            {
                foreach (Product p in c.Products)
                {
                    ...
                    csv.WriteLine("{0},{1}", arg0: p.Stock, arg1: p.Discontinued);
                }
            }
        }
    }

    WriteLine("{0} contains {1:N0} bytes.", arg0: csvFile, arg1: new FileInfo(csvFile).Length);
}
...
static void GenerateJsonFile(IQueryable<Category> categories)
{
    ...
    WriteLine("{0} contains {1:N0} bytes.", arg0: jsonFile, arg1: new FileInfo(jsonFile).Length);
}
