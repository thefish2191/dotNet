...
static void FilterAndSort()
{
    using (Northwind db = new())
    {
        ...
        IQueryable<Product> filteredProducts = processedProducts.Where(
        ...
        var projectedProducts = sortedAndFilteredProducts.Select(
            product =>
                new // anonymous type
                {
                    product.ProductId,
...
static void ProcessSettings()
{
    ...
    var appSettings = doc.Descendants("appSettings")
        ...
        .Select(
            node =>
                new { Key = node.Attribute("key")?.Value, Value = node.Attribute("value")?.Value }
        )
        .ToArray();
